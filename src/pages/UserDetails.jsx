...
const NewUser = (props) => {
  const [values, setValues] = useState({ firstName: "", lastName: "", email: "", password: "", role: "" });
  const [notify, setNotify] = useState({ show: false, type: "", message: "" });
  const [error, setError] = useState([]);

  const {data,loading}=useQuery(GET_USER,{variables:{"id":props.match.params.id}})

  useEffect(() => {
    if(data) setValues(data.getUser.user)
  }, [data]);
        ...
        <form className="ui form" onSubmit={onSubmitHandler}>
          <div className="fields">
            <div className={"six wide required field" + (error.includes("firstName") ? " error" : "")}>
              ...
              <input type="text" required placeholder={tr.first_name} value={values.firstName} name="firstName" onChange={onChangeHandler} />
            </div>
            <div className={"six wide required field" + (error.includes("lastName") ? " error" : "")}>
              <label>{tr.last_name}</label>
              <input type="text" required placeholder={tr.last_name} value={values.lastName} name="lastName" onChange={onChangeHandler} />
            </div>
            <div className="four wide field">
              <label>{tr.role}</label>
              <select name="role" value={values.role} className="ui dropdown" onChange={onChangeHandler}>
          ...
          <div className="fields">
            <div className={"six wide required field" + (error.includes("email") ? " error" : "")}>
              ...
              <input type="email" required placeholder={tr.email} value={values.email} name="email" onChange={onChangeHandler} />
            </div>
          </div>
          <div className="fields">
            <div className={"six wide required field" + (error.includes("password") ? " error" : "")}>
              <label>{tr.password}</label>
              <input type="password" required placeholder={tr.password} value={values.password} name="password" onChange={onChangeHandler} />
            </div>
          </div>
